...
export const useClickableItemsStore = defineStore("clickableItemsStore", () => {
  ...
  const getActiveItem = () => clickableItemsState.activeItem;

  const handleClickItemAction = (name: string) => {
    if (clickableItemsState.activeItem === name) Object.assign(clickableItemsState, { ...clickableItemsState, activeItem: null });
    else {
